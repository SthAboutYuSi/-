...
class TestCheckAppIdAndChannelNo(unittest.TestCase):
	"""检查AppId和渠道号接口"""
	API_URL = Config().get('OpenPlatform_API').get('ProductChannel').get('CheckAppIdAndChannelNo')
	...
	def test_CheckAppIdAndChannelNo01(self):
		"""批量验证检查AppId和渠道号接口容错"""
		datas = ExcelReader(self.excel,sheet='CheckAppIdAndChannelNo').data
		for d in range(0,len(datas)):
			if datas[d]['is_execute'] == 'N':
				...
				with self.subTest(data=datas[d]['CaseNo']+datas[d]['describe']):
					...
					res = self.client.send(params=params,headers=headers)
...
if __name__ == '__main__':
	...
	suite.addTest(TestCheckAppIdAndChannelNo('test_CheckAppIdAndChannelNo01'))
