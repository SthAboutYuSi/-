...
class TestUpdateUser(unittest.TestCase):
	"""修改账号接口"""
	API_URL = Config().get('OpenPlatform_API').get('User').get('UserUpdate')
	...
	def test_UpdateUser01(self):
		"""批量验证修改账号接口容错"""
		datas = ExcelReader(self.excel,sheet='UserUpdate').data
		for d in range(0,len(datas)):
			if datas[d]['is_execute'] == 'N':
				...
				with self.subTest(data=datas[d]['CaseNo']+datas[d]['describe']):
					...
					if datas[d]['password'] == '不输入':
						datas[d].pop('password')
...
if __name__ == '__main__':
	...
	suite.addTest(TestUpdateUser('test_UpdateUser01'))
