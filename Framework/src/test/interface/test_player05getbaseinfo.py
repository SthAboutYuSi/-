...
# @Last Modified time: 2019-08-23 15:38:33
...
class TestPlayerGetBaseInfo(unittest.TestCase):
	"""获取玩家信息接口"""
	API_URL = Config().get('OpenPlatform_API').get('Player').get('GetBaseInfo')
	...
	def test_PlayerGetBaseInfo01(self):
		"""批量验证获取玩家信息接口容错"""
		datas = ExcelReader(self.excel,sheet='GetBaseInfo').data
			...
			if datas[d]['is_execute'] == 'N':
				...
				with self.subTest(data=datas[d]['CaseNo']+datas[d]['describe']):
					...
					if 'data' in expect.keys() and expect['data']:
						...
						self.assertEqual(expect['data']['unionId'], res_unionId, msg=msg)
						self.assertEqual(expect['data']['nickname'], res_nickname, msg=msg)
						self.assertEqual(expect['data']['realName'], res_realName, msg=msg)
						self.assertEqual(expect['data']['mobile'], res_mobile, msg=msg)
						self.assertEqual(expect['data']['picture'], res_picture, msg=msg)
					self.tearDown()
...
if __name__ == '__main__':
	...
	suite.addTest(TestPlayerGetBaseInfo('test_PlayerGetBaseInfo01'))
