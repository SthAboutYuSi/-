...
class TestProductPackageCreate(unittest.TestCase):
	"""添加分包接口"""
	API_URL = Config().get('OpenPlatform_API').get('ProductPackage').get('ProductPackageCreate')
	...
	def test_ProductPackageCreate01(self):
		"""批量验证添加分包接口容错"""
		datas = ExcelReader(self.excel,sheet='ProductPackage').data
		for d in range(0,len(datas)):
			if datas[d]['is_execute'] == 'N':
				...
				with self.subTest(data=datas[d]['CaseNo']+datas[d]['describe']):
					...
					res_packageId = JMESPathExtractor().extract(query='data.packageId', body=res.text)
					res_packageCount = JMESPathExtractor().extract(query='data.packageCount', body=res.text)
					msg = '\n请求地址：'+ res.url + '\n请求数据:' + params + '\n返回结果:' + res.text
					...
					if 'data' in expect.keys() and expect['data']:
						self.assertEqual(32, len(res_packageId), msg=msg)
						self.assertEqual(expect['data']['packageCount'], res_packageCount, msg=msg)
...
if __name__ == '__main__':
	...
	suite.addTest(TestProductPackageCreate('test_ProductPackageCreate01'))
