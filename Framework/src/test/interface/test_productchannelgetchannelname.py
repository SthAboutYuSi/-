...
class TestGetChannelName(unittest.TestCase):
	"""验证获取渠道名接口 """
	API_URL = Config().get('OpenPlatform_API').get('ProductChannel').get('GetChannelName')
	...
	def test_GetChannelName01(self):
		"""批量验证获取渠道名称接口容错"""
		datas = ExcelReader(self.excel,sheet='GetChannelName').data
		for d in range(0,len(datas)):
			if datas[d]['is_execute'] == 'N':
				...
				with self.subTest(data=datas[d]['CaseNo']+datas[d]['describe']):
					...
					datas[d].pop('CaseNo')
					datas[d]['productChannelCode'] = ast.literal_eval(datas[d]['productChannelCode'])
...
if __name__ == '__main__':
	...
	suite.addTest(TestGetChannelName('test_GetChannelName01'))
