...
# @Last Modified time: 2019-09-30 18:08:50
...
class TestPlayerLoginExceptionVertifyMobile(unittest.TestCase):
	"""异常登录手机验证码接口"""
	API_URL = Config().get('OpenPlatform_API').get('Player').get('LoginExceptionVertify_mobile')
	...
	def test_LoginExceptionVertifyMobile01(self):
		"""批量验证异常登录手机验证码接口容错"""
		datas = ExcelReader(self.excel,sheet='LoginExceptionVertifyMobile').data
...
if __name__ == '__main__':
	...
	suite.addTest(TestPlayerLoginExceptionVertifyMobile('test_LoginExceptionVertifyMobile01'))
